...
$registeredEvents = supabase_request("/rest/v1/events?created_by=eq.{$_SESSION['email']}&select=*", 'GET');

// Fetch upcoming events (date greater than current date)
$upcomingEvents = supabase_request("/rest/v1/events?date=gt.$currentDate&select=*", 'GET');
